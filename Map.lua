	...
	if parentMapID and tamer.parentMapIDs[parentMapID] then
		...
		for _,questID in ipairs(tamer.questIDsByParentMapID[parentMapID]) do
			...
			if (showPaw or showInactive) and isTracked then
				...
				-- Fallbacks for clients where worldPos mapping differs (eg. MoP Classic)
				if not mapPos then
					-- try deriving worldPos from stored zone map coords, then convert to current map
					local zoneMapID, mapX, mapY = info[7], info[8], info[9]
					if zoneMapID and mapX and mapY then
						local zoneMapPos = CreateVector2D(mapX, mapY)
						local instanceID2, worldPos2 = C_Map.GetWorldPosFromMapPos(zoneMapID, zoneMapPos)
						if instanceID2 and worldPos2 then
							_, mapPos = C_Map.GetMapPosFromWorldPos(instanceID2, worldPos2, mapID)
						end
						-- final fallback: if currently on the exact zone map, use its map coords directly
						if (not mapPos) and (mapID == zoneMapID) then
							mapPos = zoneMapPos
						end
					end
				end
